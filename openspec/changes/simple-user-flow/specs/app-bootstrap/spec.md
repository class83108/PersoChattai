...
- **THEN** 系統 SHALL 依序執行：
  ...
  5. 初始化 UserRepositoryWrapper 並存入 `app.state.user_repository`
  6. 初始化 ConversationManager（注入 repository、scenario_designer、gemini_client）
  7. 掛載 FastRTC WebRTC stream 至 `/api/conversation/rtc`
  8. 啟動 ContentScheduler
