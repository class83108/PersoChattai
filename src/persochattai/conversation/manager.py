...
from persochattai.conversation.schemas import VALID_TRANSITIONS
...
class ConversationManager:
    ...
    async def start_conversation(
        ...
        await self._repository.update_status(conv_id, 'preparing')
        ...
        try:
            ...
            self.transition_state(conv_id, 'failed')
            await self._repository.update_status(conv_id, 'failed')
            return {
                'conversation_id': conv_id,
                'status': self._conversations[conv_id]['status'],
            }

        self.transition_state(conv_id, 'connecting')
        await self._repository.update_status(conv_id, 'connecting')
        ...
        self.transition_state(conv_id, 'active')
        await self._repository.update_status(conv_id, 'active')
    ...
    async def end_conversation(self, conversation_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.transition_state(conversation_id, 'failed')
            await self._repository.update_status(conversation_id, 'failed')
            return {
                'conversation_id': conversation_id,
                'status': conv['status'],
            }

        self.transition_state(conversation_id, 'assessing')
        await self._repository.update_status(conversation_id, 'assessing')
    ...
    async def cancel_conversation(self, conversation_id: str) -> dict[str, Any]:
        ...
        status = conv['status']
        if status in ('completed', 'failed', 'cancelled', 'assessing'):
            msg = f'Cannot cancel conversation in {status} state'
            raise ValueError(msg)

        if status == 'active' and len(conv['transcript']) > 0:
            await self._save_transcript_with_retry(conversation_id, conv['transcript'])
            await self._repository.update_ended_at(conversation_id, datetime.now(UTC))
        ...
        self.transition_state(conversation_id, new_status)
        await self._repository.update_status(conversation_id, new_status)
    ...
    async def simulate_time_elapsed(self, conversation_id: str, seconds: int) -> None:
        if seconds >= 13 * 60:
